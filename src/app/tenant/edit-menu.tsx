...
export default function EditMenu() {
  ...
  useEffect(() => {
    if (params.id) {
      ...
      setMaxQuota((params.maxQuota as string) || '');
      setAvailableAt((params.availableAt as string) || '');
    }
  }, [params.id]);
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      await api.updateTenantMenu(params.id as string, {
        name: name.trim(),
      ...
      Alert.alert('Sukses', 'Menu masakan berhasil diperbarui!');
      router.back();
    } catch (err: any) {
      Alert.alert('Gagal Update Menu', err.message || 'Terjadi kesalahan saat memperbarui menu.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <SafeAreaView className="flex-1 bg-slate-50 dark:bg-slate-950" edges={['top', 'left', 'right']}>
      ...
      <View className="bg-green-600 px-5 py-4 flex-row items-center">
        ...
        <Text className="text-white text-lg font-extrabold flex-1">Edit Menu Masakan</Text>
      </View>
      ...
      <ScrollView className="flex-1 p-4" showsVerticalScrollIndicator={false}>
        ...
        <View className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl p-5 mb-4 shadow-sm">
          ...
          <TextInput
            placeholder="Nasi Campur Spesial"
        ...
        <View className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl p-5 mb-4 shadow-sm">
          ...
          <TextInput
            multiline
            numberOfLines={3}
            placeholder="Tulis lauk pauk detail..."
            placeholderTextColor="#94a3b8"
            value={description}
